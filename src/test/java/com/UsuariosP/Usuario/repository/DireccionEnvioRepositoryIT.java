...
class DireccionEnvioRepositoryIT {
    ...
    @Test
    void buscarUsuarioRut(){ //Retonar solo las direcciones del usuario
        ...
        direccionEnvioRepository.save(nuevaDireccion(propietario, "Calle A", true));
        direccionEnvioRepository.save(nuevaDireccion(propietario, "Calle B" false));
        direccionEnvioRepository.save(nuevaDireccion(menor, "Calle C" true));

        List<DireccionEnvio> direcciones = direccionEnvioRepository.findByUsuarioRut(87654321L);

        assertThat(direcciones).hasSize(2);
        assertThat(direcciones).allMacth(d -> d.getUsuario().getRut().equals(87654321L));
    }

    @Test
    void buscarUsuarioRut_Activa(){ //Retornar solo la direcion activa
        Usuario propietario = usuarioRepository.save(nuevoUsuario(22334455L, "propietarioJose@gmail.com"));
        direccionEnvioRepository.save(nuevaDireccion(propietario, "Calle Activa", true));
        direccionEnvioRepository.save(nuevaDireccion(propietario, "Calle inactiva", false));

        List<DireccionEnvio> activas = direccionEnvioRepository.findByUsuarioRut(22334455L);
